...
export const CrearDenunciaForm = (setActive, toggle) => {


  return (
    <>
     <div className={ModalTitle}><h2>CREAR DENUNCIA</h2></div>
    <Formik
    initialValues={{
      Denunciante: '',
      Cedula: '',
    ...
    validate={(valores) =>{
      let errores ={};
      if(!valores.Denunciante){
        errores.Denunciante = 'Por favor ingresa un nombre completo'
      } else if(!/^[a-zA-ZÀ-ÿ\s]{1,40}$/.test(valores.Denunciante)){
        errores.Denunciante = 'El nombre solo puede contener letras y espacios'
      }
  ...
  >
      ...
      {({handleSubmit, values, touched, handleChange, handleBlur, errors})=> (
        ...
        onBlur={handleBlur}
         />
         {touched.Denunciante && errors.Denunciante && <div className={ErrorDenuncia}>{errors.Denunciante}</div>}
         </div>
        </div>
        <div>
        ...
        <div>
          ...
          <option value="Poda Ilegal">Poda Ilegal</option>
          </select>
